...
실제 로봇에서는 Go2 내장 보행 제어기(`unitree_sdk2` SportClient) 사용.

도킹 노드(aruco_go2_docking의 detector/controller)는 그대로 사용.
controller의 /cmd_vel·/joy 명령을 sport-mode 제어 명령(SportClient 호출)으로 번역하는 어댑터 한 개만 추가로 사용.
...
- **조인트 이름**: `*_thigh_joint`/`*_calf_joint`를 포함해야 컨트롤러의 `_is_sitting`/
  `_is_standing`이 매칭합니다.

## 사전 준비

1. 로봇이 **sport(고수준) 모드** — 저수준/RL 제어가 아닌 내장 보행 제어기 활성 상태.
