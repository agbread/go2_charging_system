...
이 문서 하나로 ① 실행 파일 목록·순서, ② 각 파일의 역할, ③ 환경/의존성 설정,
④ 파라미터/옵션을 모두 확인할 수 있습니다.

지원하는 실행 방식은 두 가지입니다:

| 방식 | 보행 담당 | 충전 판정 | 용도 |
|---|---|---|---|
| **A. 시뮬레이션** | RL 정책 (`rl_sar rl_sim`) | mock 노드(수동 토글) | Gazebo에서 도킹 로직 검증 |
| **B. 실기 (내장 제어기)** | Go2 공장 sport-mode 제어기 | BMS 실측 (`rt/lowstate`) | 실제 로봇 자율 충전 |
...
**2) 터미널 1 — Gazebo 환경 + 로봇 스폰**

```bash
ros2 launch aruco_go2_docking aruco_docking_sim.launch.py rname:=go2
```

**3) 터미널 2 — RL 로코모션 컨트롤러** (시뮬에는 sport 제어기가 없으므로 RL이 보행 담당)
...
### B. 실기 — Go2 내장(sport-mode) 제어기 사용

> RL 정책을 전혀 쓰지 않습니다. `go2_sport_bridge`가 도킹 컨트롤러의 명령을
> `unitree_sdk2` SportClient 호출로 번역합니다.

**0) 사전 조건 확인**

- 로봇이 **sport(고수준) 모드** — 공장 보행 제어기가 활성 상태여야 합니다.
...
**종료 조건 (자동):** 도킹 컨트롤러가 성공(`aruco_success`) 또는 최종 실패(`aruco_failed`)
판정을 내리면 스스로 종료하고, launch 이벤트 핸들러가 어댑터·검출기까지 전부 내립니다.
Ctrl+C나 pkill이 필요 없습니다.
...
| `go2_sport_bridge/src/sport_mode_adapter_node.cpp` | go2_sport_bridge (C++) | **실기 전용 어댑터.** 도킹 컨트롤러의 제너릭 명령을 sport-mode 호출로 번역: `/cmd_vel`→`SportClient::Move`, joy A→`RecoveryStand`(일어서기), B→`StandDown`(엎드림=충전 자세), X→`StandUp`, RB+DPadUp→`BalanceStand`(보행 재활성). 역방향으로 `rt/lowstate`의 관절 상태→`/joint_states`, BMS(status 6/7 또는 충전 전류)→`/charging_state` 발행. StandDown 후에는 `/cmd_vel`을 차단(엎드린 채 Move가 가면 로봇이 다시 일어나려 하기 때문). |
...
| `aruco_go2_docking/launch/aruco_docking_real.launch.py` | 실기(RL 방식용): detector + controller만 (파라미터: `docking_params_real.yaml`) |
| `go2_sport_bridge/launch/go2_native_docking.launch.py` | **실기 메인 진입점**: 어댑터 + detector + controller 일괄 실행, 종료 연동, CycloneDDS 라이브러리 경로 픽스 포함 |
| `aruco_go2_docking/config/docking_params.yaml` | 시뮬용 파라미터 (카메라 토픽 `/color/image_raw`, 데드존 보정 OFF) |
| `aruco_go2_docking/config/docking_params_real.yaml` | 실기용 파라미터 (RealSense 토픽, Go2 데드존 보정, 실측 보정값) |
...
```
                  ...
                  aruco_detector_node
                            │ /aruco/marker_pose
                            ▼
             aruco_docking_controller_node ──► /aruco_state (상위 앱 알림)
                    │            │
             /cmd_vel          /joy (A/B/X/RB+DPadUp)
                    │            │
                    ▼            ▼
     ┌─ 시뮬: rl_sar rl_sim ──────────────────┐
     └─ 실기: sport_mode_adapter_node ────────┘
                    │                    ▲
          SportClient::Move/StandDown…   │ rt/lowstate (관절+BMS)
                    ▼                    │
                 [Go2 로봇] ─────────────┘
                    │
      /joint_states, /charging_state  ──► 컨트롤러가 앉음/섬 판정·충전 판정에 사용
      (시뮬은 Gazebo joint_state_broadcaster + mock_charging_node가 대신 발행)
```
...
| ROS 패키지 | `ros-foxy-cv-bridge`, `ros-foxy-image-transport`, `ros-foxy-realsense2-camera`, geometry/sensor/std_msgs |
...
- Gazebo + `gazebo_ros`, `controller_manager`, `robot_state_publisher`, `demo_nodes_cpp`(parameter_blackboard)
- **별도 저장소 필요**: `rl_sar`(RL 컨트롤러 — `rl_sim` 실행 파일), `go2_description`(로봇 xacro)
  — 이 저장소에는 포함되어 있지 않으므로 같은 워크스페이스에 함께 빌드해야 합니다.
...
**3) DDS 환경설정 ⚠️ 가장 중요 — 빠지면 어댑터가 기동 즉시 죽습니다**
...
시뮬(`docking_params.yaml`)과 실기(`docking_params_real.yaml`) 값이 다른 항목은 `시뮬 / 실기`로 표기.
...
| `camera_pitch_deg` | 0.0 | 카메라 아래 기울기 [deg] (+ = nose-down). 실기 장착각에 맞춰 입력 |
...
| `pose_filter_alpha` / `pose_outlier_dist` | 1.0 / 0.0 (OFF) | 실기 카메라 노이즈용 EMA 필터·이상치 제거. 지터가 심하면 예: 0.3 / 0.15 |

**controller — 앉기/충전/재시도**

| 파라미터 | 값 | 설명 |
|---|---|---|
| `sit_thigh_target` / `sit_calf_target` | 1.36 / -2.65 [rad] | 엎드림 판정 관절각. **실기 보정 필요** (아래 참고) |
| `stand_calf_target` | -1.50 [rad] | 일어섬 판정 calf 각 |
| `sit_confirm_timeout_sec` | 20.0 | 관절 기반 앉음 확인 최대 대기 (실기 GetDown ~10초) |
...
**sport 어댑터 (`go2_sport_mode_adapter`, 실기 전용)**
